...
# 面试常见问题
  ...
  第一份是（停顿）漆远的无限光年，探索AI在情感陪伴游戏里的应用，主要负责角色的3C表现效果和一些Gameplay功能。
  第二份是（停顿）邢骏的创业团队，作为第一位开发，在前期为美术和策划同学快速搭建游戏框架，验证已有资产的表现效果和早期设计的玩法体验，后期主要负责游戏里的角色动画系统和一些工具链的搭建，包含算法侧动画数据的解析、压缩，UE引擎里的动画驱动逻辑和游戏开发过程中的一些工具链。
