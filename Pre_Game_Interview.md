...
1. 动画数据格式处理
   我拿到的是bin数据，首先解析成我规定的格式，boneTrack里面有keyframe number 和rotation、translation这样，根骨骼单独存一下。
2. Scale数据不存吗？
...
4. 为什么用四元数
   这个最后应该会用欧拉角传，现在用四元数只是因为插值压缩是我这边来写的，一开始方案经常变动的时候做的坐标系转换也有点浪费时间，所以ai侧先直接出四元数来避免一些软件里面坐标系的问题，后面我们重新再做一边坐标系纠正，把压缩算法内嵌到ai里面去就好，最后传输都用欧拉角，有需要的话本地再转成四元数计算。
5. 数据压缩压缩
