...
1. 动画数据格式处理
   我拿到的是bin数据，首先解析成我规定的格式，最外层根据Bone类型划分，普通的Bone数据和根骨骼数据，然后内一层工具骨骼名字划分，Bone name和Bone track，每个track里面存储这段数据每一帧的数据，所以需要keyframe number和具体的数值，比方说translate、rotation之类的。
2. Scale数据不存吗？
...
4. 为什么用四元数
   主要是因为欧拉角在数据压缩过程中的插值问题，具体表现在两个方面
...
5. 数据压缩压缩
   translate线性压缩，我写的是一个很简单的线性关键帧减少算法，通过对两帧之间线性插值，判断算出来的插值帧数据与在该位置的原始帧数据是否在误差范围内，如果在，就删除该帧数据，最后只保留不能被拟合的帧数据作为关键帧。
  这里我做了一点优化，就是保留的关键帧不停向前处理，直到遇到下一个关键帧，而不是每三个帧三个帧处理。
...
这里我补充一下压缩算法吧，以及UE4如何实现整个编码和解码流程。一般的压缩算法策略包括误差校正，降低采样率，数据编码压缩以及曲线拟合等等，UE里边基本已经实现了前三种压缩策略，我们实现了第四种并补充了其他数据压缩编码算法。
UE的实现过程是将该动画同时执行十几种压缩算法后的结果都保留下来，然后进行误差分析和择优选择，即是选择压缩成本最低精度最高的算法作为该动画的压缩策略。
压缩算法主要对应三种Encode方式固定间隔插值，可变间隔插值，通道单独压缩格式

情景、任务、行为、结果
首先，欧拉角在做插值计算的时候，会有最短路径缺失（比方说绕Z轴旋转350°和10°，实际插值10°），万向锁，角度周期性的问题。

分通道压缩的逻辑：  
首先第一步是分离动画通道：将Track数据分离成3条轨迹，包含全部骨骼的移动旋转缩放的tracklist，每个list是一根骨骼的全部动画帧pos信息和time时间信息，将帧队列转换为时间队列.
第二步：单轨道重复压缩检测：分别拆分位移旋转缩放进行压缩，对每根骨骼的所有帧信息进行扫描，如果全部动画帧数据变化幅度都小于0.0001，则将移除全部帧只保留一帧，否则什么也不做，对于有动画数据的轨迹没有任何压缩。  
第三步：移除线性帧，线性删减主要在FilterLinearKeysTemplate函数内，它是循环递归从大到小去线性检测，边检测边校正容忍值，使用了二分查找法(检测最远的点然后慢慢缩小) 。线性删减的误差校正是统一的容忍值加上末端容忍值。末端容忍值采用min和max来分别定义从叶子末端到根节点的容忍插值。最后线性删减还会模拟末端Socket的World空间和删之前的World空间的误差值，如果过大则弃用。他是通过分别给pos和rot进行Transform矩阵计算transitionSize来判断误差值。
...
第四步是动画压缩编码的实现过程，主要是根据接口先选择对应的编码格式

然后就是动画的解码，就是动画播放过程中通过解码器来读取和更新动画。
首先获取指定的解码器和时间，就能通过解码器直接从GetAnimationPose()获取动画数据。FAnimPose里面有我们需要的scale、rotation、translation数据。然后这里的数据压缩也是用了定点数压缩。

一个完整的动画周期是，原始动画数据->动画编码器（定义数据格式）（位移编码、旋转编码、缩放编码、过滤重复帧、误差矫正...）->动画数据压缩->动画序列化存储数据->动画读取数据->动画解码器（定义数据格式）（位移解码、旋转解码、缩放解码...）->动画播放。
...
数据还原成pose的过程我记得不是很清楚了，基本上都是对着源码抄的，这里面有一个问题就是四元数的转换，我传入的四元数在ue里转成欧拉角的时候发现方向是相反的，也是在源码里才发现ue的四元数转换回欧拉角的过程里面有两个参数就是反的。（记为我遇到的一个困难）
