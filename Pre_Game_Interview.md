...
### 虚函数构造过程
  ...
  1. 当一个类包含虚函数时，编译器会为该类生成一个虚函数表（vtable），虚函数表中存储了该类所有虚函数的地址。
...
虽然这与 “构造 / 析构中调用虚函数” 无直接关联，但需注意：若基类用于多态场景（如通过基类指针释放派生类对象），必须将析构函数声明为 virtual，否则会导致派生类析构函数未被调用，引发资源泄漏。
...
### shared_ptr的实现原理是什么？构造函数、拷贝构造函数和赋值运算符怎么写？shared_ptr是不是线程安全的？
  ...
  3. shared_ptr的引用计数本身是安全且无锁的，但是它里面封装的基础对象的读写不是，因此可以说shared_ptr不是线程安全的。
...
5. Lua 调用引擎函数（Lua 给引擎“下指令”）：  
   - Lua 脚本通过 `IdolAPI.Call` 等接口，传入函数名和参数（如调用 `IdolAPI.Call("MoveForward", 1.0)` 让角色移动）。  
   - **引擎通过“映射表”找到对应的真实函数，将 Lua 栈中的参数（如数字、字符串、对象引用）转换为引擎能识别的类型**（如将 Lua 的 `userdata` 转换为引擎的对象指针），最终调用引擎原生逻辑（如角色移动的 C++ 代码）。   
...
Lua 变量分为全局变量、局部变量和表的字段。默认声明的变量是全局变量，使用 `local` 关键字声明局部变量。变量类型由赋值动态决定，支持 `nil`、`number`、`string`、`boolean`、`table` 等类型。
...
Table 是 Lua 中唯一的数据结构，用于表示数组、字典、集合等。使用 `{}` 创建，支持整数和字符串索引，可动态添加/删除元素。

    ```lua
      -- 数组风格
      local fruits = {"apple", "banana"}
      print(fruits[1])  -- 输出: apple（索引从1开始）
...
Table 底层由 **数组部分** 和 **哈希部分** 组成：
    - **数组部分**：存储连续整数索引（如 `t[1]`、`t[2]`）。
    - **哈希部分**：存储非连续索引或非整数键（如 `t.name`、`t["key"]`）。

    ```lua
      local t = {10, 20, 30}  -- 数组部分长度3
      t.name = "Alice"        -- 哈希部分存储键值对
      t[5] = 50               -- 非连续索引，存入哈希部分
    ```

## 5.10 Table 如何实现高效查找？

哈希部分使用 **哈希表** 实现，通过哈希函数将键映射到槽位（bucket）：
    - 冲突解决：开放寻址法（线性探测）。
    - 动态扩容：负载因子过高时自动 rehash，调整槽位数。

## 5.11 数组索引与字典索引有何区别？

- **数组索引**：连续整数（1,2,3...），存储在数组部分，访问速度快。
- **字典索引**：非连续整数或其他类型（字符串、表等），存储在哈希部分。
...
## 5.12 Table 常用操作有哪些？*
    ...
    ```lua
      #t             -- 获取数组部分长度
      table.insert(t, value)  -- 在末尾添加元素
      table.remove(t, index)  -- 删除指定位置元素
      table.sort(t)           -- 排序数组部分
    ```
...
- 数组部分：访问、插入时间复杂度 O(1)。
- 哈希部分：平均查找、插入 O(1)，最坏 O(n)（极少见）。
- 频繁扩容会触发 rehash，影响性能。


## 5.14 ListView介绍一下

ListView这些组件再初始化添加完里面的数据后，UE会自动在UI可见的时候调用对应的回调函数。

整体流程分为：

1. 先在lua里创建 ListViewItemData，绑定上 Lua 回调，然后再用addItem添加到listview里
2. ui在滚动到对应entry可见的时候，自动触发相关回调
...
在调用 `lua_next(lua_State *L, int index);` 之前，栈顶是初始键 `nil`，下面是要遍历的表。

为什么需要 nil：lua_next的第一个参数是表，第二个参数是键。初始遍历时需要传入nil作为起始键，表示从第一个元素开始。
...
`lua_next` 首先弹出栈顶的键（初始化的时候是nil），然后将键和值依次压入栈顶，键在下面，值在上面
...
以下是针对 **Unreal Engine (UE) C++** 和 **Lua Gameplay 开发**的六大设计模式总结，结合实际场景举例说明：


### **1. 单例模式 (Singleton)**  
**定义**：确保类全局唯一实例，提供全局访问点。  
**UE/Lua 实例**：  
- **UE**：游戏中的全局管理器（如 `UGameInstance`、自定义的 `UMyGameManager`），用于管理跨关卡数据（玩家进度、配置参数），通过 `Get()` 静态方法获取唯一实例。  
- **Lua**：用模块（Module）实现单例，例如 `GameState` 模块，通过表（Table）封装唯一实例，避免重复创建。  


### **2. 工厂模式 (Factory)**  
**定义**：封装对象创建逻辑，解耦“调用者”与“具体创建实现”。  
**UE/Lua 实例**：  
- **UE**：内置的 `UObjectFactory` 基类（如 `UActorFactory`），用于在编辑器中创建自定义资产（如 `UMyItemAsset`）或 Actor（拖放预设时自动初始化组件和属性）。  
- **Lua**：通过工厂函数创建对象，例如 `CreateCharacter(type)` 根据类型（“玩家”“敌人”）生成对应实例，隐藏具体构造逻辑。  


### **3. 观察者模式 (Observer)**  
**定义**：对象间存在“一对多”依赖，当目标状态变化时，自动通知所有观察者。  
**UE/Lua 实例**：  
- **UE**：UE 的 **委托 (Delegate)** 和 **事件 (Event)** 系统，例如 `OnActorBeginOverlap` 事件，当 Actor 重叠时自动触发所有绑定的回调函数（观察者）。  
- **Lua**：通过信号槽（Signal-Slot）机制实现，如 `PlayerHealth:Connect(function(damage) ... end)`，当玩家血量变化时通知所有监听逻辑。  


### **4. 命令模式 (Command)**  
**定义**：将“请求”封装为独立对象，支持参数化、队列化、撤销/重做操作。  
**UE/Lua 实例**：  
- **UE**：编辑器的 **撤销/重做系统**（如 `FUICommand`），每个操作（移动 Actor、修改属性）被封装为命令对象，可存入队列并按需执行/回滚。  
- **Lua**：游戏中的 **技能系统**，每个技能释放作为 `Command` 对象，包含 `Execute()` 和 `Undo()` 方法，支持连招队列或技能打断回退。  


### **5. 组件模式 (Component)**  
**定义**：将对象拆解为可组合的独立组件，通过“组合”而非“继承”实现功能扩展。  
**UE/Lua 实例**：  
- **UE**：核心的 **Actor-Component 架构**，例如 `AActor` 包含 `USceneComponent`（位置）、`UMeshComponent`（模型）、`UAIControllerComponent`（AI逻辑），通过组合不同组件快速构建复杂实体。  
- **Lua**：轻量化组件设计，如角色对象由 `MovementComponent`、`CombatComponent` 等表（Table）组合而成，通过元表（Metatable）实现组件间通信。  


### **6. MVC 模式 (Model-View-Controller)**  
**定义**：分离数据（Model）、展示（View）、逻辑（Controller），降低耦合。  
**UE/Lua 实例**：  
- **UE**：UI 开发中的 **UMG + ViewModel**，例如 `UMyHUD`（View）绑定 `FPlayerViewModel`（Model），通过 `UController` 处理输入逻辑，数据变化自动同步到界面。  
- **Lua**：游戏状态管理，如 `GameModel` 存储关卡数据，`UIView` 负责渲染，`InputController` 处理用户操作并更新 Model，三者解耦。  


### **核心价值（UE/Lua 开发适配）**  
- **UE 框架原生支持**：如单例（`GameInstance`）、组件（Actor 系统）、观察者（委托）等模式已深度集成，开发者只需基于基类扩展。  
- **Lua 轻量级实现**：通过表、元表、闭包灵活实现工厂、命令等模式，避免 C++ 的复杂语法，适合快速逻辑迭代。  
- **解耦与复用**：模式的核心是“分离关注点”，例如组件模式让 UE 蓝图/代码可复用通用功能（如物理、动画），Lua 中通过模块拆分复杂逻辑。  

按需选择模式，优先利用引擎原生机制（如 UE 工厂、委托），再结合 Lua 动态特性补充自定义逻辑，可提升代码可维护性和协作效率。
...
用invoke名字更容易理解。
![invoke1](Pre_Game_Interview/2025-04-30-00-38-39.png)
![invoke2](Pre_Game_Interview/2025-04-30-00-39-51.png)

这里回调函数可以用weakptr来做，他可以让对象能够在等待被调用过程中释放，如果用strong ptr之类的一直不释放，游戏的内存会变得很大。
举个例子，如果我现在想要发起攻击，可以遍历的时候用weakptr来做，遍历的时候如果发现这个对象已经被销毁了，就不调用这个函数了。
我们回调还是用weakptr多一点，就是为了避免运行环境中的不确定性。

回答：callback叫做invoke更容易理解，他是将一个可执行函数作为参数传到一个event（function）里面，然后在这个event后续执行的过程中，激活了这个callback函数

## 7.3 数据压缩

常见的算法主要是RLE和哈夫曼编码
RLE，压缩重复字符，AAAA压缩成A4
哈夫曼编码，根据出现频率设置编码，出现频率高的字符用尽量少的位数编码来表示
![halfu](Pre_Game_Interview/2025-05-05-18-31-43.png)

## 7.4 跨平台

  大小端问题，主要是选用合适的方案，比方说DirectX12是windows的，在linux上就用不了，写的时候考虑到跨平台就注意封装

## 7.5 消息分发系统

Publish-subscribe模式，消息分发系统是一个中介者，它负责将消息从发布者传递到订阅者。发布者和订阅者之间没有直接的联系，发布者只需要将消息发送到消息分发系统，订阅者只需要'注册'自己感兴趣的消息类型即可。这样可以降低耦合度，提高系统的灵活性和可扩展性。

如果是面向对象的语言，全部用C++，可以用基类和派生类实现。
Event的扩展性，有的人用dll注入，有的用脚本语言。

这一帧的所有event全部放在event queue里，等这一帧结束了再处理。一般用ring buffer来做，ring buffer是一个循环的buffer，读写指针在buffer里循环移动。
具体表现为：有两个指针代表读写，读指针和写指针，读指针指向当前读取的位置，写指针指向当前写入的位置。每次读写操作时，读指针和写指针都会向前移动一个位置。当读指针追上写指针时，表示缓冲区为空；当写指针追上读指针时，表示缓冲区已满。
因为一帧的Event里是很多的，我们尽量不要来回的调用不同类型的event，所以我们把不同类型的event放在一起，每种类型都是一个单独的Event Queue。

Event Queue也有一点问题，1.one-frame delays，事件会延迟一帧才会被处理。比如说我现在按下了一个键，event queue里会放入一个事件，等这一帧结束了再处理这个事件。2.不能保证执行顺序，下一帧的时候同一类的消息都集合在一起，就不能保证执行顺序了。

## 7.6 热更新

脚本语言很多都是解释性语言，代码都是走到一步看一步，所以修改后就直接更新了。脚本语言自己crash不会导致引擎crash，他运行在一个虚拟机里面。
魔兽世界让lua兴起的。

脚本系统的接口，怎么接入还是比较值得思考的。

LUA Jit先执行后编译，如果没用到的分支会自动优化掉，运行时间也会减小

蓝图和脚本其实是一样的。

## 7.7 多线程

lock虽然可以解决，但是少用比较好，多用原子性操作，load、store等等
...
线程池的工作流程
初始化：创建一定数量的线程，并让这些线程进入等待状态。
任务提交：当有新任务到来时，将任务添加到任务队列Queue（需要先进先出）中，并通知一个等待的线程有新任务到来。
任务执行：线程从任务队列中取出一个任务执行，执行完毕后回到等待状态。
关闭线程池：当不再需要线程池时，停止线程池的运行，通知所有线程退出，并等待所有线程结束。

## 7.8 反射

**当我写完一行代码，引擎会告诉我说有哪些新的功能暴露出来了，如果是个变量，我就知道能够set或者get对应的东西。**
...
**原始动画数据->动画编码器（定义数据格式）（位移编码、旋转编码、缩放编码、过滤重复帧、误差矫正...）->动画数据压缩->动画序列化存储数据->动画读取数据->动画解码器（定义数据格式）（位移解码、旋转解码、缩放解码...）->动画播放**。
...
1. 第一步是分离动画通道：将Track数据分离成3条轨迹，包含全部骨骼的移动旋转缩放的tracklist，每个list是一根骨骼的全部动画帧pos信息和time时间信息，将帧队列转换为时间队列.
2. 第二步：单轨道重复压缩检测：分别拆分位移、旋转、缩放进行压缩，对每根骨骼的所有帧信息进行扫描，如果全部动画帧数据变化幅度都小于0.0001，则将移除全部帧只保留一帧，否则什么也不做，对于有动画数据的轨迹没有任何压缩。  
3. 第三步：移除线性帧，线性删减主要在FilterLinearKeysTemplate函数内，它是循环递归从大到小去线性检测，边检测边校正容忍值，使用了二分查找法(检测最远的点然后慢慢缩小) 。线性删减的误差校正是统一的容忍值加上末端容忍值。末端容忍值采用min和max来分别定义从叶子末端到根节点的容忍插值。最后线性删减还会模拟末端Socket的World空间和删之前的World空间的误差值，如果过大则弃用。他是通过分别给pos和rot进行Transform矩阵计算transitionSize来判断误差值。
  误差校正一般包含绝对误差和相对误差，UE里边采用的相对误差即是只比对当前骨骼动画数据相对于父骨骼的变化误差幅度，在一些高匹配的动画需要绝对误差分析，即是需要保留相对误差的基础上还需要验证当前骨骼离Root的误差偏移，因为不同的骨骼误差累积叠加后可能会导致绝对误差偏移很大。
4. 第四步是动画压缩编码的实现过程，主要是根据通道（位移、旋转、缩放）选择合适压缩编码方式
...
```plaintext
  原始动画数据(骨骼+帧序列)
    ↓
  分离通道: 拆分为PositionTracks, RotationTracks, ScaleTracks
    ↓
  单轨道静态检测: 移除变化极小的帧
    ↓
  线性帧过滤: 递归删除可插值生成的中间帧
...
动画播放过程中通过解码器来读取和更新动画。
首先获取指定的解码器和时间，就能通过解码器直接从GetAnimationPose()获取动画数据。FAnimPose里面有我们需要的scale、rotation、translation数据。然后这里的数据压缩也是用了定点数压缩。
...
UE 采用即时标记 - 清除（Eager Mark-Sweep）算法：
...
ComponentSpace：当前骨骼相对于Root骨骼坐标空间的Transform信息，但是注意：一般美术只有local空间和世界空间的概念，他们的local一般指bonespace，他们的世界空间一般指componentspace。
WorldSpace:程序里边真正的世界空间，以坐标原点为坐标空间，一般也用不着。

在骨骼动画帧里边一般存储的是BoneSpace的关键信息，每帧骨骼动画包含了所有骨骼的BonespaceTransform，但是一部分特殊的功能计算可能用bonespace无法推演出来，需要在ComponentSpace下进行计算，比如IK计算，需要先将BoneSpace转换到Component空间，计算完后再转换为Local空间，最终再输出动画。骨骼动画的驱动就是动画帧不断的刷新每帧BoneSpace下每根骨骼的矩阵信息，进行播放的。

数据直接填充就可以了。但是这里面有一个问题就是四元数和欧拉角的转换，我软件里欧拉角转出的四元数，再去ue里面转成欧拉角，发现有数值的方向是反的，查看源码才发现ue在四元数转欧拉角的过程里面自己添加了一些操作。
...
## 8.1 动画数据格式
  ...
  为了使整体结构更加清晰，我定义了四层结构
  第一层，区分是骨骼数据还是面部数据。
  第二层，是骨骼的额外一层，需要将Root Motion单独提取出来处理，所以这里会区分Bone数据的类型，分别存储对应的track
  第三层，根据名字区分是哪个骨骼的数据
  第四层，存储具体的数据，包括帧号和translate数据/rotation数据（rootmotion骨骼负责角色的移动，存储translate数据，其他骨骼负责旋转，存储rotation数据），面部control rig就存一个float值

## 8.2 Scale数据不存吗？

   可以存可以不存吧，我们只做一个角色，面部的scale也是固定的一套，所以没有去存，如果需要的话，也应该是在加载角色的时候设置对应的数据，不用放到数据流里传输。而且我看修改scale一般都是捏脸系统里面有这种骨骼缩放，或者一些风格化的角色、特殊效果吧。
   这个一般默认不带缩放的动画都不会提取scale，ue自己获取动画矩阵的时候默认带一帧去提取缩放矩阵，然后拿到scale数据。

## 8.3 脸部动画是怎么做的？

   我们用的是ue metahuman那一套facial rig，也就是面部的control rig，直接读取对应骨骼的数据就可以。这里我自己写了一个引擎工具，去为动画蓝图批量生成control rig变量、设置节点的引脚然后再把他们自动连接。

## 8.4 为什么用四元数

   主要是因为欧拉角的死锁问题和多义性会导致插值出来的值和实际预期值偏差很大。

   1. 欧拉角的死锁问题会引发误差，如果插值过程经过死锁区域，插值就和实际预期偏差很大了
      > 当两个欧拉角的第二次旋转角（如 Y 轴）接近 90° 时（死锁临界状态），微小的 Y 轴变化会导致第一次和第三次旋转轴（X、Z 轴）在空间中接近重合。此时，对 X、Z 轴角度的插值会被严重放大，例如：
      > 原始角度 1：(48.5557°, 82.8384°, 48.0888°)
      > 原始角度 2：(141.922°, 81.0177°, 142.027°)
      > 两者 Y 轴接近 90°，直接线性插值后的结果可能为 (5.23894°, 90.9103°, 5.05811°)，其表示的旋转方向与原始两个角度完全不同，原因是死锁导致 X、Z 轴的插值被错误映射到重合的旋转轴上。
   2. 欧拉角的多义性会导致插值没有选择最短路径，多义性是指欧拉角的角度100°与460°等价，这时候插值就会出现280°而不是100°。

## 8.5 动画数据压缩怎么做的

    首先是分通道，然后针对每个通道做一个压缩。

    基础压缩策略：线性关键帧筛选算法
      核心思想：通过**双指针滑动窗口**动态筛选关键帧，仅保留不可通过线性插值还原的必要帧
      具体操作：对每个通道的原始帧序列，使用双指针指向当前保留的前一关键帧（i-1）和后一候选关键帧（i+1），通过线性插值生成中间帧（i）的预期数据，计算与实际数据的误差
    进阶压缩：数据类型与编码优化
      四元数压缩：从 Float 到 Half 的精度平衡
        原始方案：四元数每个分量使用 32 位 Float 存储，单四元数占用 128 位（4×32bit）。
        优化方案：转换为 16 位 Half（半精度浮点数）存储，单个分量精度从 7 位有效数字降至 3 位，经测试对动画流畅度无显著影响，存储空间直接减半（128bit→64bit）。
        数学依据：动画插值依赖 SLERP 球面插值，3 位有效数字（如 0.123）在归一化四元数范围内（模长为 1）的角度误差 < 0.5°，满足实时渲染精度要求。

      定点数压缩探索（未完全落地）
        针对极致压缩场景设计的算法：
        原理：四元数归一化后，最大分量去掉以后，其余三分量绝对值≤√2/2
        所以，用 2 位标记最大分量（x/y/z/w 中的最大值，4 选 1）；
        剩余三分量用 15 位定点数存储
        效果：单四元数压缩至 48 位（2+3×15+1 位校验），较原始 Float 方案节省 62.5% 空间（128bit→48bit）。
        挑战：定点数反归一化时易累积误差，末端骨骼可能出现抖动，需配合 IK（Inverse Kinematics）算法修正，因工期限制暂未上线。
        
   首先，我是分通道压缩，每个通道都先用线性关键帧减少算法，通过对两个关键帧之间的数据线性插值，判断预期数据与实际数据的误差是否在可控范围内，如果在，就认为该数据是可以通过线性算法算出来的，删除该帧数据。然后translate就直接用lerp，rotation的四元数用GLM库的Slerp算法。
   如果想要进一步压缩，我们也可以将四元数抓为欧拉角传输、欧拉角再用定点数去压缩，但是这里如果压缩太厉害还会出现抖动的问题，需要控制在合理范围内。实际生产中，我只尝试过将float压缩为half，整体表现良好。

  这里我做了一点优化，就是用双指针指向两个保留的关键帧，不停向前处理，直到遇到下一个关键帧，而不是每三个帧三个帧处理。
  ...
  压缩我们还可以将四元数转为欧拉角，然后再用half去压缩，转欧拉角就压缩了25%，转half又压缩50%，最后是原来的37.5%。
  > 四元数压缩，插值逻辑是一样的，但是计算插值的时候用slerp，最后还原的时候也是用slerp来插值。
  > 但是四元数我们不用float来存，一个float 32bit太大了，所以我用了half来存，简单的压缩一半。
  > float 1符号位8指数位23尾数位，大概表示7位有效数字，half 1符号位5指数位10尾数位，大概表示3位有效数字，对于动画精度来讲完全够的。
  > 有效数字位数=log(10)(2^n) n是尾数的位数。
  > 当然，我们也可以用定点数压缩算法，但是这样子在一些动画片段更容易出现抖动问题，为了解决这个抖动问题又需要做一些补偿算法，而且我看每个算法都会有各自的问题，因为工期的原因没有进一步去实践和探索了。
  > 定点数压缩：我们知道一个数的范围，比如过0-100，我们就能把它映射到0-1之间，最后乘以100就能还原了。然后四元数有一个特性，当我们将四元数归一化的时候，如果我们把最大的那个值丢掉，剩下的三个值的范围就会在(负二分之根号2到正二分之根号2)之间，也就意味着，我只需要用两个bit去标记哪个分量是最大的，剩下的三个量用15bit位定点数去存储，将每一个分量的值映射到15位的数值上去，最后2+15*3=47bit，然后补1bit，变成48bit。
  > 最后还原的时候，先还原三个分量，然后用归一化反向解出来最后的值。对比原先4*32=128bit，大概是原始的37.5%，节省了62.5%的空间。
  > 如果我们都用上这个算法，那么原本一个joint，scale 1float，translation 3float，rotation 4float，压缩后scale 16bit， translation 48bit， rotation 48bit。也就是原始8float，32bytes，压缩后14bytes，节省了56.25%。
  > 
  > 再进一步优化，动画压缩的时候可能有error，然后我们动画是从一个joint传递到另一个joint，这个error就会累积，最后形成比较大的误差，而且因为每个骨架对于压缩算法的error敏感度是不一样的，所以末端就可能出现抖来抖去的情况。我们也可以做一个误差补偿，但是做得不好也会出现新的抖动，所以也有人对于末端骨骼做一个FIK（Forward Inverse Kinematics）来修正。这个IK系统我可以在后面讲一讲。

## 8.5 数据还原的时候怎么写的

  首先，需要了解引擎的动画系统，UE的所有动画节点最后的步骤都是在Evaluate里计算当前帧数值，update里设置具体的值。
  我基于AnimNode_Base重写了一个节点，在他的框架上重新实现了逻辑，主要分为三个步骤。

  1. 设置动画数据，initialize继承父类初始化，然后针对不同来源的数据做一个poselink的initialize
    前期为了节约开发成本，translate数据用lerp，rotation 四元数用slerp，后期我们也可以使用catmull-rom曲线。
    > 插值就是先找到当前关键帧数据的下一个关键帧，我保存了帧号，所以知道两帧之间相差了多少帧，然后计算出当前帧在两帧之间的比例，最后用这个比例去插值就可以了。
  2. PreUpdate，预更新动画，PreUpdate()其实就是进行常规赋值操作，我就给角色actortransform设置了一下rootmotion之类
  3. animbase其实不同于ue原生的那一套复杂的更新逻辑，我们只关注evaluate和update就可以，

  然后我们在游戏里面还原的时候，去用Catmull-Rom曲线来插值出一个平滑移动的曲线（由四个点就能定义出插值样条曲线，比起贝塞尔曲线（三阶的贝塞尔在四个点中间，构建出来经过第一个和第四个点的曲线），他通过控制点，看起来更直观。），贝塞尔曲线里面有个α，值为1的时候曲线偏离两点连出来的直线就远了，值为0的时候就出现了自相交（拐角的时候曲线自己交叉了，有一个圈），所以我们一般都用0.5.
...
## 8.6 介绍一下做了哪些gameplay功能
  
  首先，这个项目背景是围绕AI生成动作打造的一个情感陪伴游戏，通过与角色在虚拟场景的互动、表演与情感反馈，为用户提供沉浸式陪伴体验。

  主要分为局内核心玩法、局外约会互动模块、和其他部门合作的一些工具链的设计

### 局内

  1. Character 模块：接入AI全链路
      背景：为了实现角色的自然动作和表情，需要将ai输出的骨骼数据、表情数据与引擎底层架构对接。骨骼数据决定肢体动作流畅度，面部数据决定情感表达的细腻度，bone约束配置则确保动作符合物理逻辑，比方说各个关节的运动范围限制。
      工作内容：完成Ai骨骼数据、面部数据的提取、压缩、还原，编写插件导入maya的骨骼层级约束配置，确保ai生成的动作数据能够直接驱动游戏内角色模型
  2. Control & Camera：交互与视角优化
      背景：乐曲节奏标点更密集的时候，我们判断这是一个节奏快的部分，就会更频繁的切换镜头，触发一些镜头特效
      工作内容：一方面做了不同阶段的镜头切换逻辑，另一方面做了战斗过程产生的镜头特效，
  3. 备战环节：
      背景：进入战斗模式之前，会根据选择的歌曲风格生成一段舞蹈，也会根据你赠送的道具去改变舞蹈状态。
      工作内容：
        卡牌推荐：根据选择的歌曲类型和卡牌的星级，推荐更匹配的道具卡片（回血buff、特效之类的）、动作卡片（配置跳舞的动作）
        卡牌装备与卸载：支持用户手动调整卡片顺序
        卡片数值计算：基于选择的卡片属性预测一个匹配度，在最后舞蹈结束的时候再根据角色属性、使用的卡片道具等等计算演出得分。

### 局外

  背景：局外系统主要聚焦与角色的热场互动，深化角色与玩家的情感链接。

   1. 气泡选项生成；基于大语言模型预设一批对话选项，用户选择气泡后通过语音+表情+动作响应
   2. 约会拍照：这里我做了一个简单的相机拍照，有一些预设的滤镜模版（Post Process Volume），和对应的保存相片，相册展示功能，每次拍照的时候，也可以让角色换一个pose，触发约会动作模块。
   3. 约会动作：实现点击选项后的角色切换pose链路。我们会根据每个场景预设一批动作标签，每次随机打乱顺序发送给ai，ai再根据这一套标签返回给我每段链路的所有组合的完整过渡动画，然后我们在每次选择的时候其实就是去播放对应的过渡动画。
   4. 角色档案、换装：展示角色的基础信息，支持用户给其更换服装、配饰。

### 工具

  Maya效果复刻工具：
  背景：美术习惯在maya里设置绑定等操作，在ue里手动实现对应的功能会更麻烦
  工作内容：写maya插件，从根节点开始遍历每个骨骼和其相关的有效node，导出一份数据后在ue里使用原生的bone transform、constraint、还有一些计算节点复刻maya里的节点功能

  表情蓝图插件：
  背景：角色的面部表情涉及大量control rig节点，手动设置耗时且难以批量调整
  工作内容：开发ue插件，读取control rig配置设置控制节点的输入引脚，我再把每个输入符合结构拆分，批量生产float变量并且设置到对应的引脚。

## 8.7 指令系统

指令系统的核心目标是更便捷的执行一些指令

1. 初始化：创建 CCommandManager 实例，对命令管理系统进行初始化。
2. 命令创建：创建 CCommand，定义命令不同阶段的function
3. 命令添加：使用 CCommandManager:addCommand 方法将命令添加到命令管理器中。
4. 命令执行：在每一帧调用 CCommandManager:update 方法，对所有命令和队列进行更新。
5. 命令结束：当命令完成时，会自动从命令管理器中移除。

这里采用面向对象的设计思路，借助不同的类来处理不同的功能。核心类涵盖 CCommand、CCommandGroup、CCommandQueue 以及 CCommandManager。
CCommand：作为基础命令类，它定义了命令的基本属性和方法，像阶段管理、状态管理、标签管理等。
CCommandGroup：命令组类，可并行执行多个命令。
CCommandQueue：命令队列类，按照优先级依次执行命令。
CCommandManager：指令系统的管理类，负责分配队列 ID、添加和移除命令、更新命令状态等操作。

核心类实现细节
CCommand 类
  阶段管理：借助 EStage 枚举来管理命令的不同阶段，包含 Start、Execute、Finish。
  状态管理：运用 ESpecialState 枚举管理特殊状态，如 Pause、Interrupt。
  标签管理：提供 addTag、addConflictTag、hasTag 等方法来管理标签和冲突标签。
  回调绑定：_bindCallbacks 方法用于绑定不同阶段的回调函数。
  自动注册：_autoRegisterToManager 方法可将命令自动注册到 CCommandManager。
CCommandGroup 类
  命令管理：借助 mCommands 列表存储命令，mKeyCommands 表存储关键命令。
  并行执行：update 方法会更新组内所有命令的状态，当所有关键命令完成时，组命令才能完成。
CCommandQueue 类
  冲突检测：addCommand 方法会检查新命令与队列中现有命令的冲突，依据冲突处理结果移除冲突命令。
  优先级执行：update 方法每次选择优先级最高的命令执行。
CCommandManager 类
  队列管理：allocateQueueID 和 freeQueueID 方法用于分配和释放队列 ID。
  命令管理：addCommand 和 removeCommand 方法用于添加和移除命令。
  状态更新：update 方法更新所有命令和队列的状态。

## 8.8 如果你ai生成动作返回失败怎么处理的？

啊，首先ai生成动作很少有临时去生成的，所以只会出现在配卡->生成动作阶段，这部分交给算法去处理。

如果算法给我的pose有问题，那么我也应该切换到本地的动作数据。还是首先分析可能出现问题的地方，然后确定融合方案，最后总结出哪些地方需要，有哪些动作匹配度高。
...
## 9.1 简单介绍一下自己
  ...
  好的，我是黄啸，23年毕业于南京信息工程大学（停顿）与（停顿）中国科学院大学联合培养的信息工程专业，毕业以后先后参加了两份工作。
  第一份是（停顿）Remini创始人的新团队，在无限光年探索大模型在情感陪伴游戏里的应用，主要负责Gameplay功能，重点关注角色的3C表现效果。
  第二份是（停顿）原米哈游Ai lab负责人的创业团队，主要工作还是角色的3C表现效果，但是研究重点更偏向动画系统。
  另外我个人很擅长的一点就是对于新技术的主动学习和新团队的适应，我个人在GPT刚刚起步的时候就做了一个chat bot，这个项目到目前为止社群人数已经超过两千人，对于这种前沿技术我始终保持着很强的学习自驱力与学习热情，我也非常希望能够加入一个充满热情的团队一起成长，做出更优秀的作品。

## 9.2 为什么从上一家离职？

  嗯...我离职主要出于两个方向考虑。
第一，从个人职业发展角度，在游戏开发积累了一定经验后，我明确了未来要深耕某一细分领域的目标。过去工作中虽然接触过多个方向，但每个方向的探索都不够深入，难以形成系统性的专业能力。我希望能在新平台专注于某一技术或业务领域，持续提升专业深度，构建核心竞争力。
第二，从公司业务层面，随着公司战略调整，游戏业务占比逐渐降低，工作重心向其他领域转移。而我的职业兴趣和优势始终聚焦在互联网游戏产品的开发与优化上，现有工作内容与我的职业规划匹配度持续下降。为了在热爱的领域持续发展，经过慎重考虑，我决定寻找更契合自身发展方向的平台。


## 9.3 平时有什么爱好吗？

  我喜欢做探索性的事情，比方说gpt刚出的时候我就在做一些对话聊天机器人开源项目，现在qq群里人数也突破了两千，又比方说claude 3.5 Sonnet出的时候，大家对于他前端编码能力评价比较高，所以我就在vs code里面用client自己接了原生的api验证具体能力边界。
  然后日常生活方面：我旅游就是超级特种兵了，一天飞三个城市，比方说我之前早上在东极岛看日出，看完日出就飞云南，落地去去雪山大本营，凌晨两点起来爬雪山，六点下山以后再飞广州吃早茶这样。

  以前没有旅游吗？
  哈哈哈是的，三个原因吧，第一，我比较喜欢自己一个人出去玩，结伴出行的话大部分人跟我的进度会很累。第二，大学以前我没有其他经济来源，都是家里给的，会有明确的预算。第三，上大学以后，我大一到大四都是封校的，出去玩一趟非常麻烦。基于以上原因，我旅行的经历就集中在毕业以后工作的这段时间了。

## 9.4 第一份工作的空窗期

  哈哈哈，那段时间我在学校的实验室里准备了半年考研。
  因为大学最后公布成绩的时候才发现离保研名额差了一点，但是导师又给我推荐了很多其他优秀院校的研究生导师，鼓励我继续深造，所以在毕业以后我就先在实验室里准备了半年考研，可惜的是最后成绩也不高，后面和我要报考的那位导师聊了一下以后，各方面综合考虑吧，决定先去当时比较火的大模型领域做自己更擅长的AI应用，等自己考虑清楚以后再去深造。

## 9.5 工作中遇到的技术难点，如何解决的？

先说背景，我在里面做什么，为什么难，怎么解决难点，最后的效果

以表情插件编辑器过程为例（debug问题）

1. 认识问题，我需要找到蓝图里面的控制节点，设置他的引脚状态
2. 定位问题，查找蓝图节点，直接用ai之类的搜出来了，设置引脚，则是我用debug editor模式在猜测的地方打上断点，然后查看堆栈信息，找到可能调用的函数
3. 解决问题，最后我知道用什么函数，就是看引擎支不支持，怎么用的问题了，如果想调用的是private函数，那么我就直接copy一份，写一个shadow文件，把private改成public，其他变量保持不变，所以整个内存分布的方式也不变，我直接用reinterpret_cast强转。

以listview为例（陌生问题）

1. 认识问题，我首先需要知道listview是怎么运作的，问ai
2. 定位问题，在开发过程中使用断点调试，log调试去观察实际运行逻辑，查看每一个参数的信息
3. 解决问题，根据调试结果确定如何使用，在什么阶段调用等等

嗯...好的，主要分为认识问题、定位问题、解决问题三个阶段。
首先是认识问题，如果是陌生问题，我不知道怎么做的情况下，我会结合以往的经验，在国内外网站上搜索相关关键词，了解问题的大概情况以后，总结出一份prompt去问一遍各厂商的ai search模型，比方说grok、ChatGPT、Gemini、豆包这些平台他们会有一些模型的think和search模式，能够帮助我快速的了解一些摸索领域的知识。
然后是定位问题，我需要进一步深入了解遇到对的这个问题是什么，怎么发生的，如何复现等等内容。这种情况通常发生在代码报错，比方说，如果是开发过程中的报错，我会先切换到debug editor模式查询堆栈信息，定位问题出现的具体位置、具体代码；如果是发布的shipping包，我会先看程序监控软件，比方说sentry，看他抓到的奔溃信息；如果是跨平台，我会去切换到对应平台的发布版本，连接调试工具模拟运行。如果没办法直接定位到发生问题的代码，那么我就去打log日志。
最后是解决问题，一般来讲，根据之前我了解互联网信息、AI回答以及我定位到的堆栈信息，就能够解决大部分情况下出现的问题了。但如果是比较陌生的领域，到这一步还是不了解，那么就需要先和+1汇报，同步这边的工作情况，看是否需要援助，主要是为了避免耽误开发进度。如果是其他人的代码问题，那么就需要和对应的人沟通，告诉他具体是什么地方，什么情况下出现了什么错误，看我们能不能直接解决这个问题，如果问题比较复杂的话，我们判断没办法短时间内解决，那么就需要要和团队沟通，协调开发周期，确保整个信息对团队来讲是透明，同步的。
...
嗯...我个人有很强烈的意愿能够在游戏行业深耕，并最终成长为一名资深角色3C方向工程师吧，并且我把整个成长过程分为三个阶段。
在工作初期，我希望能够接触3C岗位相关的开发工作，熟悉需要掌握的技能，积累相关经验。
中期，我希望能够上手承担一部分开发，针对某一领域能够有深入的探索和实践操作。
后期，我希望能够在不断的探索中，对整个3C系统都有着丰富的开发经验，承担关键的开发岗位，能够做出让大家都满意的作品。


## 9.7 我的缺点是什么

之前工作需要做一个整体流程的时候不知道怎么做，没有全局视角，做完以后就好了

# 反问问题

## 业务相关问题

1. 岗位是新增还是替补，希望补足团队什么能力？（为什么招）
2. 如果有幸能够入职，那么我会做哪些工作？（进来以后做什么）
3. 半年之内，团队需要做的最核心的一项工作是什么，当前面临的最痛点的问题是什么？（那么我需要做什么）
4. 希望候选人具有什么样的能力？（希望招到什么样的人）

## 团队相关问题

1. 需求从执行到落地过程中，团队的工作流是什么样的？
2. 工作环节总遇到一些冲突的时候如何解决？
   1. 需求评审会的时候团队更加注重满足策划需求还是技术开发难度呢？
   2. 如果最后测试效果达不到预期，团队一般如何解决呢？
3. 团队工作氛围大概是什么样呢？平时汇报的方式和频率是什么样的呢？
