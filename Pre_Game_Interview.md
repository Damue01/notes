...
> 这里呼应了shared_ptr的构造函数和拷贝构造函数（深拷贝）的实现原理：shared_ptr的构造函数会将引用计数置为1，拷贝构造函数会将引用计数++，而不是简单地将指针拷贝过去。
...
编译阶段：对预编译文件进行处理，先进行词法分析，将字符流转化为单词序列；接着进行语法分析，根据语法规则构建语法树；然后进行语义分析，检查代码的语义正确性；最后进行优化，如常量折叠、死代码消除、循环展开等，以提高代码执行效率，生成汇编文件。
