...
在计算机科学中，反射式编程（或简称 “反射”）指一个进程能够检查、内省并修改其自身结构与行为的能力。
意味着，当我写完一行代码，系统会告诉我说有哪些新的功能暴露出来了，其他地方不需要和我链接起来就知道可以调用了。
...
一般的压缩算法策略包括误差校正，降低采样率，数据编码压缩以及曲线拟合等等
UE的实现过程是将该动画同时执行十几种压缩算法后的结果都保留下来，然后进行误差分析和择优选择，即是选择压缩成本最低精度最高的算法作为该动画的压缩策略。
