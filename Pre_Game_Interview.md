...
### 虚函数构造过程
  ...
  每个类**对象的头部都存放着一个虚指针，该虚指针指向了各自类所维护的虚函数表**，我们通过查找虚函数表中的地址来找到对应的虚函数（虚表指针）。
...
在基类的构造函数或析构函数中调用虚函数，**无法实现多态**，只会调用基类自身的版本，原因如下：  

1. **构造函数中**：  
   - 构造对象时，会先调用基类构造函数，再初始化子类成员。此时子类部分尚未构造，对象在基类构造阶段的类型是“基类对象”，而非完整的子类对象。因此，即使子类重写了虚函数，此时调用的仍是基类版本（编译器会进行静态绑定）。  

2. **析构函数中**：  
   - 析构对象时，会先调用子类析构函数，再调用基类析构函数。在基类析构阶段，子类的成员已经被销毁，对象的类型退化为“基类对象”，此时调用虚函数同样只会触发基类版本。  

简单总结：在基类构造/析构期间，对象的类型尚未完全形成（构造时子类未构造，析构时子类已销毁），虚函数的动态绑定机制失效，只能调用基类自己的实现。因此，**应避免在构造函数和析构函数中调用虚函数**，以免出现预期外的行为。
...
## 8.5 数据压缩怎么做的
   我实际上用的是简单的线性关键帧减少算法，通过对两个关键帧之间的数据线性插值，判断预期数据与实际数据的误差是否在可控范围内，如果在，就认为该数据是可以通过线性算法算出来的，删除该帧数据。然后translate就直接用lerp，rotation的四元数用GLM库的Slerp算法。
