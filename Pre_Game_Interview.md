...
当基类析构函数是虚函数的时候，通过基类指针删除派生类对象会触发动态绑定：先调用派生类的析构函数，再自动调用基类的析构函数。
若基类析构函数非虚，派生类对象通过基类指针释放时，只会调用基类析构函数，导致派生类资源未释放，引发泄露
...
vector数组，插入删除会影响容器重新分配内存，导致原先的迭代器失效。
set/map，底层是红黑树，插入不会影响其他迭代器，删除也只会让指向被删除元素的迭代器失效，其他迭代器保持有效。

典型场景：循环中删除元素未正确更新迭代器

```cpp
  vector<int> vec = {1, 2, 3, 4};
  for (auto it = vec.begin(); it != vec.end(); ++it) {
      if (*it == 2) {
          //错误做法 vec.erase(it); // 此处erase后it失效，直接++会导致未定义行为
          it = vec.erase(it); // 正确做法，erase会自动返回指向删除位置的下一个元素的迭代器
      }
  }
```
...
### 什么是内存碎片，内存碎片是在虚拟内存还是物理内存？
  ...
  内存碎片：采用分区式存储管理的系统，在储存分配过程中产生的、不能供用户作业使用的主存里的小分区称成“内存碎片”。（就是分配后不能用的那部分内存？）
  **内存碎片只存在于虚拟内存上**
...
Lua 变量分为**全局变量、局部变量和表**的字段。默认声明的变量是全局变量，使用 `local` 关键字声明局部变量。变量类型由赋值动态决定，支持 `nil`、`number`、`string`、`boolean`、`table` 等类型。
...
- **Lua**：我的一些UI也是单例管理的，主要是便于其他代码调用

### 工厂模式 (Factory)

**定义**：封装对象创建逻辑，将**调用**和**具体实现**解耦开来
**UE/Lua 实例**：  

- **UE**：内置的 Actor（拖放预设时自动初始化组件和属性）
...
Overlap 关注“是否重叠”，适用于静态范围检测。
SweepTest 关注“移动中是否碰撞”，适用于动态物体的轨迹检测。

#### Overlap（重叠检测）

- **定义**：检测两个或多个碰撞体是否已经重叠（相交）。
- **用途**：
  - 触发区域（如触发器盒子）。
  - 检测物体是否在某个范围内。
  - 角色拾取物品时的碰撞判断。
- **特点**：
  ...
  - 不考虑物体的移动轨迹，只检查最终位置。
- **示例函数**：
  - `Actor->IsOverlappingActor()`
  - `GetWorld()->OverlapMultiByChannel()`

#### SweepTest（扫掠测试）

- **定义**：模拟一个碰撞体从起点到终点的移动过程，检测在移动路径上是否与其他物体发生碰撞。
- **用途**：
  - 移动过程中的碰撞检测（如子弹轨迹、角色移动）。
  - 预测物体移动时是否会撞到障碍物。
- **特点**：
  - 考虑物体的移动轨迹和体积，能检测到“沿途”的碰撞。
  - 返回碰撞的时间点（Time of Impact，TOI）和位置。
- **示例函数**：
  - `GetWorld()->SweepSingleByChannel()`
  - `UKismetSystemLibrary::BoxSweepMulti()`
...
## 9.1 简单介绍一下自己
  ...
  另外我个人很擅长的一点就是对于新技术的主动学习和新团队的适应，我个人在GPT刚刚起步的时候就做了一个chat bot，这个项目到目前为止社群人数已经超过两千人，
  目前，我希望加入一个充满创新热情的团队，一起成长，做出更优秀的作品。
